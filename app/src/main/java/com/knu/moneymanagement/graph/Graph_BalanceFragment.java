...
public class Graph_BalanceFragment extends Fragment {

    private LineChart lineChart;

    public Graph_BalanceFragment() {
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_graph_balance, container, false);
    ...
    private void createLine(int year) {
        ...
        ArrayList<Entry> balance = new ArrayList<>(); //차트 데이터 셋에 담겨질 데이터
        ...
        for (int i = 0; i < 12; i++) {
            ...
            balance.add(new Entry(i, monthBalance[i]));
        }

        LineDataSet lineDataSetBalance = new LineDataSet(balance, "잔액"); //LineDataSet 선언
        lineDataSetBalance.setColor(Color.DKGRAY); //LineChart에서 Line Color 설정
        lineDataSetBalance.setCircleColor(Color.DKGRAY); // LineChart에서 Line Circle Color 설정
        lineDataSetBalance.setDrawValues(false);


        LineData lineData = new LineData(); //LineDataSet을 담는 그릇 여러개의 라인 데이터가 들어갈 수 있습니다.
        lineData.addDataSet(lineDataSetBalance);
