...
public class Graph_AcculBalanceFragment extends Fragment {

    private LineChart lineChart;

    public Graph_AcculBalanceFragment() {
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_graph_accul_balance, container, false);

        lineChart = root.findViewById(R.id.lineChart4);
    ...
    public void createLine(int year) {
        ...
        long[] monthBalance = new long[12];

        String[] labels = {"1월", "2월", "3월", "4월", "5월", "6월", "7월", "8월", "9월", "10월", "11월", "12월"}; // x축 라벨
        ArrayList<Entry> acculBalance = new ArrayList<>(); //차트 데이터 셋에 담겨질 데이터
        ...
        for (int i = 0; i < 12; i++)
            monthBalance[i] = monthIncome[i] - monthExpen[i];

        acculBalance.add(new Entry(0, monthBalance[0]));
        for (int i = 1; i < 12; i++) {
            monthBalance[i] += monthBalance[i - 1];
            acculBalance.add(new Entry(i, monthBalance[i]));
        }

        LineDataSet lineDataSetAcculBalance = new LineDataSet(acculBalance, "누적 잔액"); //LineDataSet 선언
        lineDataSetAcculBalance.setColor(Color.DKGRAY); //LineChart에서 Line Color 설정
        lineDataSetAcculBalance.setCircleColor(Color.DKGRAY); // LineChart에서 Line Circle Color 설정
        lineDataSetAcculBalance.setDrawValues(false);

        LineData lineData = new LineData(); //LineDataSet을 담는 그릇 여러개의 라인 데이터가 들어갈 수 있습니다.
        lineData.addDataSet(lineDataSetAcculBalance);
